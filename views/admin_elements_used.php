...
if (!empty($used)){
    foreach ($used as $element){
        ...
        if ($element->sortorder < count($used)){
            $actions .= "&nbsp;<a href=\"view.php?id={$cm->id}&amp;view=admin&amp;what=lowerelement&amp;elementid={$element->id}\"><img src=\"".$OUTPUT->pix_url('down', 'mod_tracker')."\" /></a>";
        } else {
            $actions .= "&nbsp;<img src=\"".$OUTPUT->pix_url('down_shadow', 'mod_tracker')."\" />";
        }
        $actions .= "&nbsp;<a href=\"view.php?id={$cm->id}&amp;view=admin&amp;what=editelement&amp;elementid={$element->id}&amp;used=1\"><img src=\"".$OUTPUT->pix_url('edit', 'mod_tracker')."\" /></a>";
        ...
        $actions .= "&nbsp;<a href=\"view.php?id={$cm->id}&amp;view=admin&amp;what=removeelement&amp;usedid={$element->id}\"><img src=\"".$OUTPUT->pix_url('right', 'mod_tracker')."\" /></a>";
        if ($element->active){
            $actions .= "&nbsp;<a href=\"view.php?id={$cm->id}&amp;view=admin&amp;what=setinactive&amp;usedid={$element->id}\"><img src=\"".$OUTPUT->pix_url('hide', 'mod_tracker')."\" /></a>";
        } else {
            $actions .= "&nbsp;<a href=\"view.php?id={$cm->id}&amp;view=admin&amp;what=setactive&amp;usedid={$element->id}\"><img src=\"".$OUTPUT->pix_url('show', 'mod_tracker')."\" /></a>";
        }
