<?php //$Id: restorelib.php,v 1.4 2011-10-14 20:38:24 vf Exp $
...
*/
    ...
    function tracker_restore_mods($mod, $restore) {
        ...
        if ($data) {
            ...
            $tracker->defaultassignee = backup_todb($info['MOD']['#']['DEFAULTASSIGNEE']['0']['#']);
            $tracker->subtrackers = backup_todb($info['MOD']['#']['SUBTRACKERS']['0']['#']);
    ...
    function tracker_elements_restore_mods($old_tracker_id, $new_tracker_id, $info, $restore, $new_course_id) {
        ...
        if (!empty($elements)){
            for($i = 0; $i < sizeof($elements); $i++) {
                ...
                $element->course = backup_todb($element_info['#']['COURSE']['0']['#']);  
				...
				// We have to ignore all already existing element on base of name and type, for shared elements
				if ($element->course == 0){
					if ($ignored = get_record('tracker_element', 'name', $element->name, 'type', $element->type)){
						$IGNORED_ELEMENTS[$oldid] = $ignored;
						continue;
					}
				}

				// TODO : secure the new name by appending indexing suffix
				if ($intheway = get_record('tracker_element', 'name', $element->name)){
				}
    ...
    function tracker_elementitems_restore_mods($old_tracker_id, $new_tracker_id, $info, $restore) {
        ...
        if (!empty($elementitems)){
            for($i = 0; $i < sizeof($elementitems); $i++) {
                ...
                // Discard element items that have been ignored (already existing globals) (based on element->oldid)
                $ignoredids = array_keys($IGNORED_ELEMENTS);
                if (in_array($elementitem->elementid, $ignoredids)){
                	continue;
                }
    ...
    function tracker_elementuseds_restore_mods($old_tracker_id, $new_tracker_id, $info, $restore) {
        ...
        $elementsuseds = @$info['MOD']['#']['ELEMENTUSEDS']['0']['#']['ELEMENTUSED'];

        //Iterate over used elements
        if (!empty($elementsuseds)){
            for($i = 0; $i < sizeof($elementsuseds); $i++) {
                $elementused_info = $elementsuseds[$i];
                ...
                if ($element) {
                	...
                	// this might be the case for a discarded element, we have to remap it to an existing element
                	if (in_array($elementused->elementid, array_keys($IGNORED_ELEMENTS))){
                		$ignored = $IGNORED_ELEMENTS[$elementused->elementid];
                		$existing = get_record('tracker_element', 'name', $ignored->name, 'type', $ignored->type);
                		$elementused->elementid = $existing->id;
                	}
                }
    ...
    function tracker_issueownerships_restore_mods($old_tracker_id, $new_tracker_id, $info, $restore) {
        ...
        if (!empty($ownerships)){
            for($i = 0; $i < sizeof($ownerships); $i++) {
                ...
                //We'll need this later!!
                $oldid = backup_todb($ownership_info['#']['ID']['0']['#']);
